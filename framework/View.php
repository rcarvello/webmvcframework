...
class View
{
    ...
    /**
     * Parses for errors
     * @param $errors array containing the error's messages
     * @return void
    ...
    public function xssCleanString($string, $charset = CHARSET)
    {
     ...
     /**
     * Check for {STATICTPL:*} placeholders and creates an array of them.
      *
     */
    private function checkStatics()
    {
        $regexForGlobalPlaceHolder = "/\{(STATICTPL:.*?)\}/s";
        @preg_match_all($regexForGlobalPlaceHolder, $this->tpl, $result);
        $this->staticsPlaceHolders = array_merge($this->staticsPlaceHolders,$result[1]);
    }

    /**
     * Applies the replacements to {STATICTPL:*} placeholders
     */
    private function replaceWithStatics()
    {
        if (!empty($this->staticsPlaceHolders)) { 
            foreach ($this->staticsPlaceHolders as $placeholder) {
                    ...
                    $this->tpl = str_replace($variableName, $value, $this->tpl);    
            }
        }        
    }
