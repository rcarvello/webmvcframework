...
<sup>Supported PHP versions from 5.6 to 8.5</sup>
...
- MySQL table names should use **snake_case** (for example `category`, `order_item`, `customer_address`).
- This convention keeps generated ORM classes predictable and aligned with framework conventions.

Table-to-class mapping:
- Each table is mapped to one autogenerated Bean class in `models/beans`.
- Example: table `category` -> autogenerated class `BeanCategory`.
- Another example: table `order_item` -> autogenerated class `BeanOrderItem`.

Concrete example (`category`):
...
Generated Bean correspondence (`models/beans/BeanCategory.php`):

```php
class BeanCategory extends MySqlRecord implements Bean
{
      // Field attributes mapped from table columns
      private $categoryId;     // category_id
      private $categoryName;   // category_name
      private $listOrder;      // list_order

      // Main generated methods
      ...
      public function getTableName() {}   // returns "category"
      public function getDdl() {}         // returns table DDL
